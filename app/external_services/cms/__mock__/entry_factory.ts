...
export const entryApiFieldsFactory = Factory.define<EntryApiFields>(({ sequence }) => {
  return {
    title: '2022年の振り返り',
...
export const entryFactory = Factory.define<Entry>(({ afterBuild }) => {
  setMockDate();
  afterBuild(() => {
    resetMockDate();
  });
  ...
  const entrySchema = entryApiFieldsFactory.build();
